...
update visualization_outer_params_target_view_info
set target_ds_id = (select ccv.table_id
                    from visualization_outer_params_target_view_info tvi
                             INNER JOIN core_chart_view ccv on tvi.target_view_id = ccv.id)
where target_view_id = (select ccv.id
                        from visualization_outer_params_target_view_info tvi
                                 INNER JOIN core_chart_view ccv on tvi.target_view_id = ccv.id);
