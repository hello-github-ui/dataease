...
ON TABLE `area`
    IS '地图区域表';
COMMENT
ON TABLE `core_area_custom`
    IS '自定义地图区域信息表';
COMMENT
ON TABLE `core_chart_view`
    IS '组件图表表';
COMMENT
ON TABLE `core_dataset_group`
    IS '数据集分组表';
COMMENT
ON TABLE `core_dataset_table`
    IS 'table数据集';
COMMENT
ON TABLE `core_dataset_table_field`
    IS 'table数据集表字段';
COMMENT
ON TABLE `core_dataset_table_sql_log`
    IS 'table数据集查询sql日志';
COMMENT
ON TABLE `core_datasource`
    IS '数据源表';
COMMENT
ON TABLE `core_datasource_task`
    IS '数据源定时同步任务';
COMMENT
ON TABLE `core_datasource_task_log`
    IS '数据源定时同步任务执行日志';
COMMENT
ON TABLE `core_de_engine`
    IS '数据引擎';
COMMENT
ON TABLE `core_driver`
    IS '驱动';
COMMENT
ON TABLE `core_driver_jar`
    IS '驱动详情';
COMMENT
ON TABLE `core_menu`
    IS '路由菜单';
COMMENT
ON TABLE `core_opt_recent`
    IS '可视化资源表';
COMMENT
ON TABLE `core_rsa`
    IS 'rsa 密钥表';
...
ON TABLE `core_store`
    IS '用户收藏表';
COMMENT
ON TABLE `core_sys_setting`
    IS '系统设置表';
COMMENT
ON TABLE `data_visualization_info`
    IS '可视化大屏信息表';

COMMENT
ON TABLE `visualization_background`
    IS '边框背景表';
COMMENT
ON TABLE `visualization_background_image`
    IS '背景图';
COMMENT
ON TABLE `visualization_link_jump`
    IS '跳转记录表';
COMMENT
ON TABLE `visualization_link_jump_info`
    IS '跳转配置表';
COMMENT
ON TABLE `visualization_link_jump_target_view_info`
    IS '跳转目标仪表板图表字段配置表';
COMMENT
ON TABLE `visualization_linkage`
    IS '联动记录表';
COMMENT
ON TABLE `visualization_linkage_field`
    IS '联动字段';
COMMENT
ON TABLE `visualization_subject`
    IS '主题表';
COMMENT
ON TABLE `visualization_template_extend_data`
    IS '模板图表明细信息表';
