...
import {clearAllCopilot, copilotChat, copilotFields, getDatasetTree, getListCopilot} from '@/api/dataset'
...
<template>
    <div class="copilot">
        ...
        <div class="copilot-service">
            <div class="dialogue">
                <div :style="copilotDialogueComputed" class="copilot-dialogue">
                    <DialogueChart key="isWelcome" isWelcome></DialogueChart>
                    <DialogueChart v-for="ele in historyArr" :key="ele.id" :copilotInfo="ele"></DialogueChart>
                    <DialogueChart v-if="copilotChatLoading" key="isAnswer" isAnswer></DialogueChart>
                </div>
                <div ref="questionInputRef" :class="overHeight && 'over-height'" class="question-input">
                    <el-input
                        ref="inputRef"
                    ...
                    <el-icon v-if="copilotChatLoading" class="copilot-icon circular-input_icon">
                        <Icon name="icon_loading_outlined">
                            <icon_loading_outlined class="svg-icon"/>
                        </Icon>
                    </el-icon>
                    <el-icon v-else :class="isActive && 'active'" class="copilot-icon" @click="queryAnswer">
                        <Icon
                        >
                            <component
            ...
            <div :style="{ width: showLeft ? 0 : '280px' }" class="dataset-select">
                <el-tooltip :content="t('relation.retract')" effect="dark" placement="left">
                ...
                <el-tooltip :content="t('relation.expand')" effect="dark" placement="left">
                ...
                <div v-show="!showLeft" style="width: 100%; height: calc(100% - 16px)">
                    <div class="title-dataset_select">{{ t('copilot.choose_dataset') }}</div>
                    <div class="tree-select" style="margin: 0 16px">
                        <el-tree-select
                            ref="treeSelectRef"
                            v-model="datasetId"
                            :data="computedTree"
                        ...
                        >
                            <template #default="{ node, data }">
                                <div class="content">
                                    ...
                                    <span :title="node.label" class="label ellipsis" style="margin-left: 8px">{{
                                            node.label
                                        }}</span>
                                </div>
                            </template>
