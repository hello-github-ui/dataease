...
<template>
    <div class="mobile-dashboard">
        <van-sticky>
            ...
            />
            <div v-if="directName.length" class="direct-name-arr">
                <div :key="t('work_branch.dashboard')" @click="onClickPanel">
                ...
                <div v-for="(ele, index) in [...directName]" :key="ele" @click="handleDir(index)">
          <span :class="index !== directName.length - 1 && 'primary-name'" class="label ellipsis">{{
                  ele
              }}</span>
        ...
        <div :class="!!directName.length && 'dashboard-cell-group-tab'" class="dashboard-cell-group">
            <div v-if="!directName.length" class="dashboard-cell-group_filter">
                <el-input
                    v-model="filterText"
                ...
                <el-dropdown trigger="click" @command="sortTypeChange">
                    <el-icon class="filter-icon-span">
                        <Icon v-if="curSortType.includes('asc')" class="opt-icon" name="dv-sort-asc"
                        >
                            <dvSortAsc class="svg-icon opt-icon"
                            />
                        </Icon>
                        <Icon v-if="curSortType.includes('desc')" class="opt-icon" name="dv-sort-desc"
                        >
