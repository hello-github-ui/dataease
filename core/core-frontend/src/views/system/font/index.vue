...
import {defaultFont, deleteById, edit} from '@/api/font'
...
<template>
    <div v-loading="loading" class="font-management_system">
        ...
        <div class="font-content_overflow">
            <div v-if="fontListComputed.length" class="font-content_list">
                <div v-for="ele in fontListComputed" :key="ele" class="font-content_item">
                    <span v-if="ele.isDefault" class="font-default">{{ t('system.default_font') }}</span>
                    <div class="font-name">
            <span :class="!ele.isBuiltin && 'font-name_text'" :title="ele.name">{{
                    ele.name
                }}</span>
                    ...
                    <div class="font-upload_btn">
                        ...
                        >{{ t('system.replace_font_file') }}
                        </el-button
                        >
                        <el-button v-if="!ele.isDefault" secondary @click="setToDefault(ele)">{{
                                t('system.as_default_font')
                            }}
                        </el-button>
                        <el-button v-if="ele.id !== '1'" secondary @click="deleteFont(ele)">{{
                                t('common.delete')
                            }}
                        </el-button>
                    </div>
            ...
            <div v-else style="height: 178px; margin-top: 142px">
                <empty-background :description="$t('work_branch.relevant_content_found')" img-type="tree"/>
            </div>
        </div>
    </div>
    <UploadDetail ref="uploadDetail" @finish="uploadFilish"></UploadDetail>
</template>
