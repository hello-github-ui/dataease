...
import {computed, inject, nextTick, onBeforeMount, ref, toRefs, watch} from 'vue'
...
<template>
    <div class="white-nowrap">
        <div class="filed" @mouseleave="showDel = false" @mouseover="showDel = true">
            ...
            <div v-if="item.fieldId" class="white-nowrap flex-align-center" style="position: relative">
                ...
                <template v-if="item.filterType === 'logic'">
                    ...
                    <template v-if="authTargetType === 'sysParams'">
                        <el-select v-model="item.value" class="w70 mar5" size="small">
                            <el-option
            ...
            "
                    ...
                    <template v-else-if="!['null', 'empty', 'not_null', 'not_empty'].includes(item.term)">
                        ...
                        <el-input v-else v-model="item.value" class="w70 mar5" size="small"/>
                        <div class="bottom-line"></div>
                    </template>
                </template>
                ...
                >
                    <template #reference>
                        <el-input ref="enumInput" v-model="item.enumValue" clearable readonly size="small">
                        </el-input>
                    </template>
                    <div class="de-panel clearfix">
                        <div class="mod-left">
                            <el-input v-model="filterFiled" :placeholder="t('auth.enter_keywords')"></el-input>
                            <ul class="autochecker-list" style="height: 260px">
                                ...
                                <el-checkbox-group v-model="checkboxlist" @change="selectedChange">
                                    <el-scrollbar height="230px">
                                        <li v-for="i in checkListWithFilter" :key="i" :title="i">
                                            <el-checkbox :label="i">
                                                {{ i }}
                                            </el-checkbox>
                                        </li>
                    ...
                    {{ t('auth.manual_input') }}
                        ...
                        ></textarea>
                                                <div class="text-area-btn">
                                                    <button class="btn" type="button" @click="showTextArea = false">
                                                        <span>{{ t('auth.close') }}</span>
                                                    </button>
                                                    <button class="btn right-add" type="button" @click="addFields">
                  ...
                  ><span @click="clearAll">{{ t('user.clear_button') }}</span>
                                ...
                                <el-scrollbar height="230px">
                                    <li v-for="(i, idx) in checklist" :key="i">
                                        <div :title="i" class="right-checkbox__label">{{ i }}</div>
                                        <el-icon class="remove-hover-icon" @click="delChecks(idx, i)">
                                            <Icon>
