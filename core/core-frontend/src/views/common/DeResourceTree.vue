...
import {copyResource, deleteLogic, queryShareBaseApi, ResourceOrFolder} from '@/api/visualization/dataVisualization'
...
<template>
    <div class="resource-tree">
        <div class="tree-header">
            <div v-show="showPosition === 'preview'" class="icon-methods">
                <span class="title"> {{ resourceLabel }} </span>
                <div v-if="rootManage" class="flex-align-center">
                    <el-tooltip :content="t('work_branch.new_folder')" effect="dark" placement="top">
                        <el-icon
                    ...
                    <el-tooltip :content="newResourceLabel" effect="dark" placement="top">
                                ...
                                <el-dropdown-menu>
                                    <el-dropdown-item @click="addOperation('newLeaf', null, 'leaf', true)">
                                        <el-icon :class="`handle-icon color-${curCanvasType}`">
                                            <Icon>
                                                <component :is="dvSvgType" class="svg-icon"></component>
                                            </Icon>
                                        </el-icon>
                                    ...
                                    <el-dropdown-item @click="addOperation('newFromTemplate', null, 'leaf', true)">
                                        <el-icon class="handle-icon">
                                            <Icon name="dv-use-template">
                                                <dvUseTemplate class="svg-icon"/>
                                            </Icon>
                                        </el-icon>
            ...
            <el-dropdown trigger="click" @command="handleSortTypeChange">
                <el-icon class="filter-icon-span">
                    <el-tooltip :content="sortTypeTip" :offset="16" effect="dark" placement="top">
                        <Icon v-if="state.curSortType.includes('asc')" class="opt-icon" name="dv-sort-asc"
                        >
                    ...
                    <el-tooltip :content="sortTypeTip" :offset="16" effect="dark" placement="top">
                        <Icon v-if="state.curSortType.includes('desc')" class="opt-icon" name="dv-sort-desc"
                        >
        ...
        <el-scrollbar v-loading="copyLoading" class="custom-tree">
          ...
          <span class="custom-tree-node">
            ...
            <el-icon v-else-if="curCanvasType === 'dashboard'" style="font-size: 18px">
              <Icon
              ><component
            ...
            <el-icon v-else class="icon-screen-new color-dataV" style="font-size: 18px">
              <Icon name="icon_operation-analysis_outlined"
              ><icon_operationAnalysis_outlined class="svg-icon"
              /></Icon>
            </el-icon>
            <span :title="node.label" class="label-tooltip">{{ node.label }}</span>
            <div v-if="data.weight >= 7 && showPosition === 'preview'" class="icon-more">
              <el-icon
                  v-if="data.leaf"
                  class="hover-icon"
