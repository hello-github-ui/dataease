...
import {computed, h, nextTick, onMounted, reactive, ref, shallowRef, unref, watch} from 'vue'
import {dsTypes} from '@/views/visualized/data/datasource/form/option'
import type {ElMessageBoxOptions, TabPaneName} from 'element-plus-secondary'
import {ElAside, ElButton, ElIcon, ElMessage, ElMessageBox, ElScrollbar} from 'element-plus-secondary'
...
import type {Node, SyncSetting} from './form/option'
...
<template>
    <div v-loading="dsLoading" class="datasource-manage">
        <ArrowSide
        ...
        >
            ...
            <div class="resource-tree">
                <div class="tree-header">
                    <div class="icon-methods">
                        ...
                        <div v-if="rootManage" class="flex-align-center">
                            <el-tooltip :content="t('deDataset.new_folder')" effect="dark" placement="top">
                                <el-icon
                            ...
                            <el-tooltip :content="t('datasource.create')" effect="dark" placement="top">
                    ...
                    <el-dropdown trigger="click" @command="handleSortTypeChange">
                        <el-icon class="filter-icon-span">
                            <el-tooltip :content="sortTypeTip" :offset="16" effect="dark" placement="top">
                                <Icon class="opt-icon" name="dv-sort-asc"
                                >
                            ...
                            <el-tooltip :content="sortTypeTip" :offset="16" effect="dark" placement="top">
                                <Icon class="opt-icon" name="dv-sort-desc"
                                >
                ...
                <el-scrollbar ref="scrollbarRef" class="custom-tree" @scroll="handleScroll">
                    <el-tree
                        v-if="dsListTreeShow"
                        ref="dsListTree"
              ...
              <span class="custom-tree-node" style="position: relative">
                <el-icon :class="data.leaf && 'icon-border'" style="font-size: 18px">
                  ...
                  ><component :is="getDsIconName(data)" class="svg-icon"></component
                  ></Icon>
                </el-icon>
                <el-icon
                ...
                <div v-if="data.weight >= 7" class="icon-more">
                  <handle-more
                      v-if="!data.leaf"
        ...
        >
            <template v-if="!state.datasourceTree.length && mounted">
                <empty-background :description="t('data_source.no_data_source')" img-type="none">
                    <el-button v-if="rootManage" type="primary" @click="() => createDatasource()">
                        <template #icon>
                            <Icon name="icon_add_outlined">
            ...
            <template v-else-if="!!nodeInfo.id">
                <div class="datasource-info">
                    <div class="info-method">
                        <el-icon class="icon-border">
                            ...
                            >
                                <component :is="iconDatasourceMap[nodeInfo.type]" class="svg-icon"></component
                                >
                            </Icon>
                        </el-icon>
              ...
              {{ t('visualization.create_by') }}:{{ nodeInfo.creator }}
                        ...
                        <el-popover :offset="8" placement="bottom" show-arrow trigger="hover" width="290">
                            <template #reference>
                                <el-icon class="create-user" size="16px">
                        ...
                        <div class="right-btn flex-align-center">
                            <el-button v-permission="['dataset']" secondary @click="createDataset(null)">
                                <template #icon>
                            ...
                            <el-button v-else-if="nodeInfo.weight >= 7" type="primary" @click="editDatasource()">
                                <template #icon>
                                    <Icon name="icon_edit_outlined">
                                        <icon_edit_outlined class="svg-icon"/>
                                    </Icon>
                ...
                "
                            ...
                            >
                                <template #default="scope">
                                    <el-tooltip :content="t('data_set.a_new_dataset')" effect="dark" placement="top">
                                        <el-button
                                    ...
                                    <el-tooltip :content="t('visualization.details')" effect="dark" placement="top">
                ...
                "
                            >
                                <el-row v-show="nodeInfo.configuration.urlType !== 'jdbcUrl'" :gutter="24">
                                    <el-col :span="12">
                    ...
                    <el-icon>
                      ...
                      ></Icon>
                                ...
                                <template v-if="showSSH">
                                    <el-row v-if="nodeInfo.configuration.useSSH" :gutter="24">
                                        <el-col :span="12">
                  ...
                  >{{ t('datasource.priority') }}
                    ...
                    >
                        <div v-if="slotProps.active" class="api-card-content">
                            <div v-for="api in nodeInfo.apiConfiguration" :key="api.id" class="api-card">
                                <el-row>
                                    <el-col :span="19">
                    <span class="name">
                      <span :title="api.name" class="ellipsis">{{ api.name }}</span>
                    </span>
                                    ...
                                    <el-col :span="5" style="text-align: right">
                                        <el-button text @click="updateApiTable(api)">
                                            <template #icon>
                                ...
                                <div class="req-value">
                                    ...
                                    <el-tooltip :content="api.url" effect="dark" placement="top">
                                        <span>{{ api.url }}</span>
                                    </el-tooltip>
                                </div>
                            </div>
                        </div>
                        <el-button class="update-records" text @click="updateApiDs">
                            <template #icon>
                                <icon name="icon_replace_outlined">
                                    <icon_replace_outlined class="svg-icon"/>
                                </icon>
            ...
            "
                    ...
                    >
                        ...
                        <el-button class="update-records" text @click="getRecord">
                            <template #icon>
                                <icon name="icon_describe_outlined"
                                >
                                    <icon_describe_outlined class="svg-icon"
        ...
        <EditorDatasource ref="datasourceEditor" @refresh="refresh"></EditorDatasource>
        <el-dialog
            v-model="userDrawer"
        ...
        >
            ...
            <el-scrollbar>
                <el-table
                    ...
                    style="width: 100%"
                >
                    <el-table-column :label="t('datasource.column_name')" prop="originName"/>
                    <el-table-column :label="t('datasource.field_type')" prop="type">
                        <template #default="scope">
        ...
        <creat-ds-group ref="creatDsFolder" @finish="saveDsFolder"></creat-ds-group>
        <el-drawer
            v-model="showRecord"
        ...
        >
            ...
            >
                <el-table-column :label="t('datasource.data_table')" prop="name"></el-table-column>
                <el-table-column :label="t('datasource.sync_rate')" prop="triggerType">
                    <template #default="scope">
                        <div class="flex-align-center">
                ...
                <el-table-column :label="t('datasource.start_time')" prop="startTime">
                    <template v-slot:default="scope">
                        <span>{{ timestampFormatDate(scope.row.startTime) }}</span>
                    </template>
                </el-table-column>
                <el-table-column :label="t('datasource.end_time')" prop="endTime">
                    <template v-slot:default="scope">
                        <span>{{ timestampFormatDate(scope.row.endTime) }}</span>
                    </template>
                </el-table-column>
                <el-table-column :label="t('data_source.update_result')" prop="taskStatus">
                    <template #default="scope">
                        <div class="flex-align-center">
                            ...
                            >
                                ...
                                {{ t('dataset.error') }}
                                <el-icon class="error-info" @click="showErrorInfo(scope.row.info)">
