...
import {checkRepeat, isShowFinishPage, latestUse, loadRemoteFile, save, update, validate} from '@/api/datasource'
import {Base64} from 'js-base64'
import type {ApiConfiguration, Configuration, SyncSetting} from './option'
import {dsTypes, nameMap, typeList} from './option'
    ...
    >
        <template #header="{ close }">
            ...
            <div v-if="!editDs" class="editor-step flex-center">
                <el-steps :active="activeStep" align-center space="150px">
                    <el-step>
                        <template #icon>
                            <div class="step-icon">
                <span class="icon">
            ...
            <el-icon class="datasource-close" @click="close">
                <Icon name="icon_close_outlined">
                    <icon_close_outlined class="svg-icon"/>
                </Icon>
            </el-icon>
        </template>
        <div v-loading="dsLoading" class="datasource">
            <div v-if="!editDs" class="ds-type-select">
                <div class="title">
                    <el-input
                        v-model="filterText"
            ...
            <span class="custom-tree-node flex-align-center">
              <el-icon v-if="!!data.catalog" class="icon-border" style="width: 18px; height: 18px">
                ...
                <Icon v-else
                ><component :is="iconDatasourceMap[data.type]" class="svg-icon"></component
                ></Icon>
              </el-icon>
            ...
            <div :class="editDs && 'edit-ds'" class="ds-editor">
                <div v-show="activeStep !== 0 && !editDs" class="ds-type-title">
                    {{ typeTitle }}
                </div>
                <div :class="(activeStep === 0 || editDs) && 'type-title'" class="editor-content">
                    <ds-type-list
                        v-show="activeStep === 0"
