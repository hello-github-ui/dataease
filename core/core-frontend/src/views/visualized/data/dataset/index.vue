...
import {computed, h, nextTick, onBeforeMount, provide, reactive, ref, shallowRef, unref, watch} from 'vue'
...
<template>
    <div v-loading="dtLoading" :class="isIframe && 'de-100vh'" class="dataset-manage">
        <ArrowSide
        ...
        >
            <template v-if="!state.datasetTree.length && mounted">
                <empty-background :description="t('data_set.data_set_yet')" img-type="none">
                    <el-button v-if="rootManage" type="primary" @click="() => createDataset()">
                        <template #icon>
                            <Icon name="icon_add_outlined">
                                <icon_add_outlined class="svg-icon"/>
                            </Icon>
                        </template>
                        {{ t('deDataset.create') + t('auth.dataset') }}
                    </el-button
                    >
                </empty-background>
            </template>
            <template v-else-if="!!nodeInfo.id">
              ...
              {{ t('visualization.create_by') }}:{{ nodeInfo.creator }}
                        ...
                        <div class="right-btn">
                            <el-button v-permission="['panel']" secondary @click="createPanel('dashboard')">
                                <template #icon>
                            ...
                            <el-button v-permission="['screen']" secondary @click="createPanel('dvCanvas')">
                                <template #icon>
                            ...
                            <el-button v-if="nodeInfo.weight >= 7" type="primary" @click="editorDataset">
                                <template #icon>
                ...
                <div class="dataset-table-info">
                    ...
                    <template v-if="['dataPreview', 'structPreview'].includes(activeName)">
                        <div :class="[{ 'struct-preview': activeName === 'structPreview' }]" class="info-table">
        ...
        ></de-resource-group-opt>
        <creat-ds-group ref="creatDsFolder" @finish="getData()"></creat-ds-group>
    </div>
    <!--导出数据集弹框-->
    <el-dialog
        v-if="showExport"
